...
def load_model():
    ...
    # return model
    load_path = Path("data/ADU/models") / lang / "Stacking" / "joblib"
    load_name = "Stacking6.pkl"
    path = load_path / load_name
    model = joblib.load(path)
    return model
...
def load_clpr_model():
    ...
    # model = joblib.load(path)
    # return model
    load_path = Path("data/ADU/models") / lang / "clpr" / "Stacking" / "joblib"
    load_name = "Stacking2.pkl"
    path = load_path / load_name
    model = joblib.load(path)
    return model
...
def print_metrics(targets, predictions):
    ...
    print("Precision:\t%8.8f" % precision_score(targets, predictions,average="macro"))
    print("Recall:\t%8.8f" % recall_score(targets, predictions,average="macro"))
    print("F1-Score:\t%8.8f" % f1_score(targets, predictions,average="macro"))
    return (
...
def save_mc_model(model, params):
    """Save model and parameters to file."""
    if params is not None:
        param_dir = (
            Path("data")
