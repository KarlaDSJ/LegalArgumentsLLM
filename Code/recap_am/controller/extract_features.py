...
if config["nlp"]["embeddings"] == "sentence_transformers":
    if lang == "de":
        ...
        # modeles transformers plus élevé
        #word_embedding_model = models.Transformer("sentence-transformers/all-mpnet-base-v2")
        word_embedding_model = models.Transformer("nlpaueb/bert-base-uncased-echr")
...
def embedding(span):
    ...
    if config["nlp"]["embeddings"] == "spacy":
        ...
        #for embeders like berd use this
        embed = embedder.encode(str(span)).tolist()
